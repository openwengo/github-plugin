...
public class ReleaseGHEventSubscriber extends GHEventsSubscriber {
    private static final Logger LOGGER = LoggerFactory.getLogger(ReleaseGHEventSubscriber.class);
    ...
    protected Set<GHEvent> events() {
        return immutableEnumSet(RELEASE);
    }

    /**
     * Calls {@link GitHubPushTrigger} in all projects to handle this hook
     *
     * @param event   only RELEASE event
     */
    @Override
    protected void onEvent(final GHSubscriberEvent event) {
        GHEventPayload.Release release;
        try {
            release = GitHub.offline().parseEventPayload(new StringReader(event.getPayload()), GHEventPayload.Release.class);
        } catch (IOException e) {
            LOGGER.warn("Received malformed ReleaseEvent: " + event.getPayload(), e);
            return;
        }
        URL repoUrl = release.getRepository().getUrl();
        final String releaseName = release.getRelease().getName();
        LOGGER.info("Received ReleaseEvent for {} from {}", repoUrl, event.getOrigin());
        ...
        if (fromEventRepository == null) {
            ...
            URL htmlUrl = release.getRepository().getHtmlUrl();
            fromEventRepository = GitHubRepositoryName.create(htmlUrl.toExternalForm());
            if (fromEventRepository != null) {
                LOGGER.debug("ReleaseEvent handling: 'html_url' field "
